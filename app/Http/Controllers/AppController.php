...
class AppController extends Controller
{
    ...
    public function sendMessage(Request $request)
    {
        ...
        //if statements for bank
        if ($replay == "Thx broer ik stuur je wat geld") {
            $oldSaldo = User::select('saldo')->where('id', $account_id)->first();
            $newSaldo = $oldSaldo['saldo'] + 50;
            User::where('id', $account_id)->update(['saldo' => $newSaldo]);
            $notification = new Notifications();

            $notification->message = "Er is geld bijgeschreven op uw bank";
            $notification->account_id = $account_id;
            $notification->save();
            $notification = Notifications::select('message')->where('account_id', Auth::id())->latest()->first();
        }
       
        if ($replay == "Ewa ik heb €60 gestuurd voor de 2 zakken") {
            $oldSaldo = User::select('saldo')->where('id', $account_id)->first();
            $newSaldo = $oldSaldo['saldo'] + 60;
            User::where('id', $account_id)->update(['saldo' => $newSaldo]);
            $notification = new Notifications();
